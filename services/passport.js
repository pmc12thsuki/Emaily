...
// 這個 function 定義當有 request 進來時，如何透過 cookie 解密後得到的 userid，找出該 user instance
...
passport.use(new GoogleStrategy({
  ...
  // we must call done() in our passport callback after finished our work
  // done() take two argument, first is error, second is the user we find
  const existingUser = await User.findOne({ googleId: profile.id });
  if (existingUser) { // find this user
